...
auto Trie::Get(std::string_view key) const -> const T * {
  ...
  std::shared_ptr<const TrieNode> trie_node = this->root_;
  if (trie_node == nullptr) {
    return nullptr;
  }
  for (char c : key) {
    auto temp = trie_node->children_.find(c);  // this is a bug that hard to find, I worked for several days
    if (temp == trie_node->children_.end()) {
      return nullptr;
    }
    trie_node = temp->second;  // const map can't get the key, "at" should be used
  }
  if (!trie_node->is_value_node_) {
    return nullptr;
  }
  const auto *node_with_value = dynamic_cast<const TrieNodeWithValue<T> *>(trie_node.get());
  ...
  for (char c : key) {
    ...
    if (iter == cur->children_.end()) {
      if (count == static_cast<int>(key.size())) {
        cur->children_.insert(
            std::pair<char, std::shared_ptr<const TrieNode>>(c, std::make_shared<TrieNodeWithValue<T>>(value_ptr)));
        break;
      }
