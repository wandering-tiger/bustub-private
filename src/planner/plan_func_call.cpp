...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  bustub::StringExpressionType type;
  if (func_name == "lower") {
    type = bustub::StringExpressionType::Lower;
  } else if (func_name == "upper") {
    type = bustub::StringExpressionType::Upper;
  } else {
  ...
  std::shared_ptr<StringExpression> expression = std::make_shared<StringExpression>(args[0], type);
